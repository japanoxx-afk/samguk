...
namespace SamKookFreeNet {
  ...
  static class GameQualityPatch {
    public static byte[] Apply(byte[] input,bool rally,bool timer) {
      ...
      int va=BitConverter.ToInt32(input,opt+28)+rva;
    ...
    static void Hook(byte[] b,int o,int target,byte[] expected,byte opcode){Check(b,o,expected);b[o]=opcode;Put(b,o+1,target-(0x400000+o+5));}
