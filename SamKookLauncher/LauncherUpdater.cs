...
static class LauncherUpdater {
  static readonly string[] Files={"SamKookLauncher.exe","LauncherUpdater.exe","GameMonitor.exe","MapPreviewRenderer.exe","cnc-ddraw.dll","cnc-ddraw-LICENSE.txt","README.md"};
 [STAThread] static int Main(string[] args) {
  ...
  try {
   ...
   foreach(string name in Files) {
    ...
    if(name=="GameMonitor.exe" || name=="LauncherUpdater.exe" || name=="MapPreviewRenderer.exe" || name.EndsWith(".dll")) using(var f=new FileStream(dest,FileMode.Open,FileAccess.ReadWrite,FileShare.None)) { }
