...
namespace SamKookFreeNet {
 ...
 static class SyncSafetyPatch {
  public static string Profile(bool latency,bool selection,bool rice,bool observer) {
   return "sync-safety-3;latency="+latency+";selection36="+selection+";rice="+rice+";observer2="+observer;
  }
  ...
  public static string ReadReport(string path) {
   ...
   if(BitConverter.ToUInt32(b,o)!=0x314e5953 || (schema!=1 && schema!=2))throw new InvalidDataException("동기화 기록 형식 오류");
   ...
   if(reason==5)s.AppendLine("stateHash="+BitConverter.ToUInt32(b,o+40).ToString("X8")+" peerHash="+BitConverter.ToUInt32(b,o+48).ToString("X8"));
